...
- **Plusieurs sondes, une propriété par surface** — le format le plus sûr à saisir : `SONDE_facade_no` = `49.54, 1.10, 333` (« lat, lon [, orientation [, inclinaison [, albedo]]] », nombres à **point** décimal ; deux valeurs = à plat, trois = mur vertical orienté, quatre = inclinaison précisée). La propriété `SONDES` — tableau JSON équivalent sur une ligne — reste acceptée et se cumule. Sept façades du même point ne coûtent qu'un appel à la source par mesure ; mémoire d'état, notification ntfy et événements IFTTT sont tenus **par sonde** (`sonde_facade_no_plein_soleil`…).
